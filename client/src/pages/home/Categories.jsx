        ...
        {data.map((e,index) => (
          ...
          >
            <img
              src={e.image}
              alt=""
              className={`h-full w-[100%] transition-all duration-300 ${index === data.length-1 ? 'object-contain' : 'object-cover'}`}
            />
            <span className="category absolute text-center top-0 left-0 right-0 bottom-0 flex justify-center items-center z-20 md:text-lg font-bold text-xs">
              {e.name}
            </span>
          </Link>
        ))}
