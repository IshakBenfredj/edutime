...
const Contact = () => {
    ...
    const handleSend = async (e) => {
        e.preventDefault()
        if (message.name.length === 0 || message.email.length === 0 || message.subject.length === 0 || message.message.length === 0) {
        ...
        <section className="contact center gray-bg" id='contact'>
            <div className="container-contact box-shadow center">
                ...
                <form className="contact-form" onSubmit={handleSend}>
                    ...
                    <div className={`wrap-input1`}>
                        <input
                            className={`input1  ${isEmptyEmail ? 'border-danger' : 'border-0'}`}
                            type="email"
                            value={message.email}
                            onChange={handleChange}
                            name="email"
                    ...
                    <div className={`wrap-input1`}>
                        <input
                            className={`input1  ${isEmptySubject ? 'border-danger' : 'border-0'}`}
                            type="text"
                    ...
                    <div className={`wrap-input1`}>
                        <textarea
                            className={`input1  ${isEmptyMessage ? 'border-danger' : 'border-0'}`}
                            name="message"
