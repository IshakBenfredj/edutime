...
export default function ChangeRequests() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const reqFilter = data.filter((d) => d.type === "change");
  ...
  return (
    <div className="pt-20 pb-16 bg-bgcolor min-h-screen">
      <div className="container">
        <Title title={"طلبات التغيير"} />
        {loading ? (
          <Loading />
        ) : requests.length === 0 ? (
          <Empty text={"لا يوجد طلبات تغيير جديدة"} />
        ) : (
...
const Request = ({ req, setRequests, requests }) => {
  ...
  const handleAccept = async () => {
    try {
      ...
      handleSuccess(`تم قبول طلب التغيير الخاص ب ${user.name}`);
  ...
  const handleRefuse = async () => {
    try {
      ...
      handleSuccess(`تم رفض طلب التغيير الخاص ب ${user.name}`);
      const newReqs = requests.filter((r) => r._id !== req._id);
      setRequests(newReqs);
      await createNotification({
        userTo: user._id,
